...
*&---------------------------------------------------------------------*
*& Purpose: Gateway for PA infotype 0009 subtype 0 via HR_INFOTYPE_OPERATION.
*& Clean Core: encapsulate SAP HR API; no standard modifications.
*& Optional CI fields ZBANKL/ZBANKN on P0009: assign here if required.
*&---------------------------------------------------------------------*
CLASS zcl_hr_bank_infotype DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! MOD from sy-datum to high date; fills standard BANKL/BANKN (and Z* if in CI).
    ...
    "! DEL using full P0009 (validity from screen / database row).
...
CLASS zcl_hr_bank_infotype IMPLEMENTATION.
  ...
  METHOD modify_salary_bank.
    ...
    " Mirror to customer include fields if present on P0009 (adjust names to CI).
    " ASSIGN COMPONENT 'ZBANKL' OF STRUCTURE ls_p0009 TO FIELD-SYMBOL(<zbankl>).
    " IF sy-subrc = 0. <zbankl> = iv_bankl. ENDIF.
